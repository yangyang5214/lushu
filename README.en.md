...
- **Notes** — every stop can carry a short free-text note, up to 20 characters (parking, tickets, contacts…), and so can each “Day N” header on a multi-day trip (plan, lodging, reminders). Notes are saved and synced with the book, and show up on read-only shared pages too.
