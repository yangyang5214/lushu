...
- **备注**：每个地点都能写一句自定义文字，最多 20 字（停车、门票、联系人等）；多天行程里「第 N 天」后面也能写一句（当天安排、住宿、提醒）。备注随路书一起保存、同步，分享出去的只读页面里也能看到。
...
- **云游动画**：路书名下方的「云游」把整本路书播成一段驾车动画（Remotion）：高德静态地图为底，车沿高德路网按路线顺序走完，期间逐天切换、沿途停靠点逐个亮起，片头片尾给出全程里程与时长；拖动进度条可回看任意一段。只读分享里也能放。弹窗右上角可以把这段动画**下载成 mp4**（浏览器本地逐帧编码，服务端不参与）。
...
| 地图 | 高德 JS API（底图 / 标记 / 图面）· 高德 Web 服务 API（POI 检索 / 周边检索 / 驾车规划 / 静态地图） |
| 动画 | Remotion Player（驾车动画的逐帧渲染，见 `src/lib/drive.ts`） |
...
| 名称 | 必填 | 用途 |
| --- | --- | --- |
| `AMAP_KEY` | 是（搜索 / 路线） | 高德「Web 服务」key，**不是**上面那个 JS API key。`/api/places`（搜索）、`/api/poi`（点地图看地点）、`/api/route`（驾车规划）与 `/api/staticmap`（云游底图）都只用高德，未配置时这几个接口返回 5xx。支持用 `;` 配多个（`key1;key2`）：按顺序轮换分摊配额，单个失败自动换下一个。 |
...
- **点地图看地点**：点中底图上的 POI 招牌时，高德 JS API（`hotspotclick`）直接给出那个点的 id，卡片就是它（和高德官网一样「点哪个是哪个」）；点空白处则靠坐标推断——逆地理（v3 geocode/regeo）认出「这一点属于哪个地方」（车站 / 景区 / 商场这类大点），周边检索（v5 place/around）给评分 / 图片和旁边的小店，合并后挑出排第一的那张卡。半径随视野放大；结果坐标是 GCJ02（底图坐标系），查询本身不写库，只有用户选「加入路线」时才转成 WGS84 落进路书；同一点附近的结果缓存 1 天。
- **驾车节流**：所有驾车请求先过一道 3 次/秒的排队闸口（`AMAP_DRIVE_QPS`），避免自己把自己打出上游限流；遇 10004 / 10020 按退避重试并换 key。
- **云游的底图对齐**：静态地图不能在前端直接调（Web 服务 key 不下发），走 `/api/staticmap` 代理，只收镜头中心和级别。高德静态地图的瓦片基准是 **512px**（不是 JS API 的 256px），实测过：把 `paths` 画到静态图上与 `src/lib/drive.ts` 的墨卡托投影比，对角偏移不到 1px —— 换成 256px 假设会差出十几像素，路线就浮在路网旁边了。
- **云游导出 mp4**：`@remotion/web-renderer` 在浏览器里把同一份 composition 逐帧画进 canvas，再用 WebCodecs 编成 h264/mp4（封装走 mediabunny），全程不联网、不登录，服务端没有渲染任务。这个包（含编码器）体积不小，所以只在点「导出」时才 `import()`：不导出就不加载。两个小细节是为了导出和预览长得一样：车图标用 `?inline` 钉成 data URL（这一层 SVG 导出时会被整个序列化成一张图片，而「当图片用的 SVG」不加载外部资源，车会消失）；导出期间会先暂停预览，免得两边抢 CPU。
...
- 云游导出 mp4 走浏览器的 WebCodecs：Chrome / Edge 等较新的浏览器可用，Safari / Firefox 上会提示「浏览器不支持导出」。导出时页面会忙一阵（按里程算，一段 30 秒的片子通常几十秒）。
